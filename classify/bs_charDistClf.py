...
class bs_charDistClf(Classifier):
    ...
    def __init__(self):
        ...
        self.clf = linear_model.LogisticRegression(class_weight='balanced', solver='lbfgs', multi_class='auto', max_iter=5000)
    ...
    def fit(self, data):
        ...
        self.features = feat_df.iloc[:, 1:].values
        # training the classifier
        self.clf.fit(self.features, self.labels)

    def predict_training(self, folds=3):    # 5 default folds
    ...
    def predict(self, data):
        ...
        """
        feat_df = data[['value']].copy()
        feat_df = feat_df.astype(str)   #change df to string
        feat_df['length'] = feat_df['value'].apply(lambda val: len(val))
        feat_df['digit_frac'] = feat_df['value'].apply(
            lambda val: 0 if len(val) == 0 else
            sum(char.isdigit() for char in val) / len(val))
