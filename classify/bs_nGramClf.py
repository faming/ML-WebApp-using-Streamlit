...
class bs_nGramClf(Classifier):

    """Classify data-points using counts of n-gram sequence of words or chars.

    The bs_nGramClf uses n-grams of words or characters (based on user
    ...
    Attributes:
        ...
        vectorizer (object): Vectorizer for transform
        
        ing text to features. It
    ...
    def __init__(self, ngram_range=(1, 1), analyzer='word', count=True,
        ...
        if count:
            self.vectorizer = CountVectorizer(analyzer=analyzer,
                                              ...
                                              max_features=n_features)  #CountVectorizer(lowercase=False)
        else:
    ...
    def fit(self, data):
        ...
        self.lrm = linear_model.LogisticRegression(class_weight='balanced', solver='lbfgs', multi_class='auto', max_iter=5000)
        self.lrm.fit(self.features, self.labels)

    def predict_training(self, folds=3):
        ...
        """
        partial_clf = linear_model.LogisticRegression(class_weight='balanced', solver='lbfgs', multi_class='auto', max_iter=5000)
    ...
    def predict(self, data):
        ...
        """
        values = list(str(item) for item in data['value'])  #change all items to string, modified 7/7
