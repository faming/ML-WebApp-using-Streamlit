"""
Implement ln_matcher.

This module is the main module of the ln_matcher package and implements the
ln_matcher class.
...
class ln_matcher:
    ...
    The ln_matcher learns to match an input schema to a mediated schema.
    The class considers panda dataframes as databases and their column names as
    the schema. ln_matcher learn to do schema matching by training on
    ...
    def __init__(self, dataframes, mappings, sample_size=300):
        ...
        unigram_count_clf = clf.bs_nGramClf(ngram_range=(1, 1))
        bigram_count_clf = clf.bs_nGramClf(ngram_range=(2, 2))
        unichar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                ngram_range=(1, 1))
        bichar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                               ngram_range=(2, 2))
        trichar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                ngram_range=(3, 3))
        quadchar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                 ngram_range=(4, 4))
        char_dist_clf = clf.bs_charDistClf()
        ...
        self.classifier_type = ['value', 'value', 'value', 'value',
                                'value', 'value', 'value']
        if self.data_src_num > 3:   # default 5
            col_char_dist_clf = clf.bs_charDistClf()
            col_trichar_count_clf = clf.bs_nGramClf(ngram_range=(1, 1))
            col_quadchar_count_clf = clf.bs_nGramClf(ngram_range=(2, 2))
            col_trichar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                        ngram_range=(3, 3))
            col_quadchar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                         ngram_range=(4, 4))
            col_quintchar_count_clf = clf.bs_nGramClf(analyzer='char_wb',
                                                          ngram_range=(5, 5))
            col_word_count_clf = \
                clf.bs_nGramClf(analyzer=utils.columnAnalyzer)
            knn_clf = \
                clf.bs_knnClf()
    ...
    def make_prediction(self, data):
        ...
        for column in list(data):
            ...
            # print('score dimension is ', scores.shape)
            ...
            max_to_mean = flat_scores.max() - sum(flat_scores)/len(flat_scores)
            secmax_to_mean = sorted(flat_scores)[-2] - sum(flat_scores)/len(flat_scores)
            confi = (max_to_mean - secmax_to_mean)/max_to_mean
            confi = "%.1f%%" % (100 * confi)
            # print(column, 'prediction confidence', confi) 
            predicted_output[column] = [self.columns[max_ind], confi]
        return predicted_output

    def save_model(self, name):
        """Serializes the ln_matcher object into a model file using python's
        picke library."""
